...
Authored IDs are Unicode-free protocol tokens matching `[A-Za-z][A-Za-z0-9._:~-]{0,127}`. IDs are unique in their declared namespace. A workload defines a machine ID plus program and thread IDs; a program defines parameter and source-block IDs. Each thread ID begins with its machine ID and `:`, and run-scoped event IDs use the same machine prefix. This is the M1 form of the machine/guest namespace required by later models. User-facing names and labels are separate bounded text fields and must be rendered as text, never HTML.

Generated event IDs, causal-parent IDs, core entity IDs and trace-boundary IDs use the same token alphabet with a 160-character maximum, preserving room for the full machine prefix and a checked 64-bit sequence. Mixed entity arrays admit both authored thread IDs and generated core IDs. T03 generates event IDs as <machineId>:event:<sequence>; core IDs begin as <machineId>:core:<index> and append colon characters until distinct from every declared thread ID. See docs/decisions/t03-contract-corrections.md for the precise bound, deterministic collision handling and pre-release compatibility correction. Authored ID bounds are unchanged.
...
Snapshots are authoritative and carry a sequence. Every thread state carries `blockId`, the immutable source block at its instruction pointer or lifecycle boundary: the first block before arrival; the next/current block while ready, dispatching, or running; the `ioWait` block while blocked; the executed `end` block after normal termination; the offending block after a thread error; and the next/current block when censored. This state is authoritative even when detailed events are absent or truncated. Snapshots are emitted only at stable boundaries after all four T01 phases for a tick have closed; they never expose an intra-phase state. Consequently, a nonempty ready queue cannot coexist with an idle core. Every queue and core reference resolves to exactly one listed thread, and the bidirectional status/ownership constraints hold.
...
Errors distinguish validation/protocol failure, unsupported schema/model/engine/random algorithm, resource limits, arithmetic overflow, control-budget exhaustion, invalid state transitions, checkpoint failure, and internal engine failure. Pre-ledger `invalidCommand`, `unsupportedSchema`, `unsupportedModel`, `unsupportedEngine`, `unsupportedRandomAlgorithm`, and `resourceLimit` failures use sequence `"0"` and `terminal: false` because no run exists. `unsupportedProtocol` follows routing step 2. Ledger-scoped `invalidState` and `checkpointFailed` use the current ledger sequence and are nonterminal.

Runtime `arithmeticOverflow`, `controlBudgetExceeded`, and `internalEngine` use the current ledger sequence, are terminal, and carry the authoritative final snapshot in the same reply before the run ID is retired. That snapshot has matching sequence and `status: "error"`, an empty ready queue, idle cores, the offending thread in `error`, every other unfinished thread in `censored`, and already terminated threads unchanged. Attributable arithmetic/control errors require `threadId` and `blockId` in the error payload, matching the offending thread's final snapshot state and runtime-error event. A thread-attributed internal error also supplies both; a global internal error supplies neither and censors every unfinished thread. Thus the fatal boundary and source remain inspectable without a later `inspect`, including after trace truncation. Runtime errors use T01's run-fatal accounting. `terminal` always describes an existing run, never merely a rejected request; transport errors do not silently turn into completed runs. Only terminal runtime errors carry a snapshot, and `receivedProtocolVersion` appears only on `unsupportedProtocol`.

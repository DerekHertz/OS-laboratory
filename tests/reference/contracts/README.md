...
Valid cases independently exercise all M1 blocks, a maximum `u64` tick without JSON-number conversion, create and selected-thread advance commands, a base-plus-large-sequence delta, a source-attributed runtime error with its final snapshot under trace truncation, a stable-boundary typed event snapshot, bound checkpoint completion, explicit full resynchronization, and unsupported-protocol/random-algorithm errors encoded in the supported response protocol.

Invalid cases challenge unknown schema/protocol versions, JSON-number ticks, `u64` overflow, duplicate block IDs, invalid parameter ranges, missing termination, unknown program/operand references, ambiguous command payloads, unknown delta operations, mismatched or source-less events, ghost queue entries, blockless authoritative thread state, a terminal runtime error without source/final state, bad pre-ledger error metadata, broken thread/core ownership, and a base-plus-delta result that violates stable-boundary ownership. Additional generated negative controls challenge unsupported engine/random-algorithm versions, nesting depth 65, 10,001 recursively counted blocks, documents over 1 MiB, and sequence gaps.
